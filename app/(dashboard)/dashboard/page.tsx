...
async function getStats(session: any, searchParams?: any) {
  try {
    ...
    const sessionStats = {
      ...
      lanjutRagu: filteredVisits.filter((v: any) => (v.h1 === 'Lanjut' && v.h2 === 'Ragu-ragu') || (v.h1 === 'Ragu-ragu' && v.h2 === 'Lanjut')).length,
      tidakRagu: filteredVisits.filter((v: any) => (v.h1 === 'Tidak Lanjut' && v.h2 === 'Ragu-ragu') || (v.h1 === 'Ragu-ragu' && v.h2 === 'Tidak Lanjut')).length,
    };
        ...
        <div className="db-hero-banner">
          ...
          <div className="db-hero-visual">
            ...
            <div className="db-hero-avatar">
              <span>{(session?.name || "U").charAt(0).toUpperCase()}</span>
            </div>
          </div>
        </div>
...
/* ─── Attendance Chart ─────────────────── */
function AttendanceChart({ label, present, absent, color = "#10b981" }: { label: string; present: number; absent: number; color?: string }) {
    ...
    <div className="db-attendance-card">
      <div className="db-attendance-header">
        <div className="db-attendance-title">
          <div className="db-attendance-dot" style={{ background: color }}></div>
          Kehadiran {label}
        </div>
        <div className="db-attendance-badge" style={{ color, background: `${color}18` }}>
      ...
      <div className="db-attendance-body">
        <div className="db-attendance-chart">
          <svg viewBox="0 0 36 36" style={{ width: '100%', height: '100%', transform: 'rotate(-90deg)' }}>
            <circle cx="18" cy="18" r="15.915" fill="transparent" stroke="var(--border)" strokeWidth="3"></circle>
            <circle cx="18" cy="18" r="15.915" fill="transparent" stroke={color} strokeWidth="3.5"
              strokeDasharray={`${presentPercent} ${100 - presentPercent}`} strokeDashoffset="0"
              style={{ transition: 'stroke-dasharray 0.8s ease', strokeLinecap: 'round' }}></circle>
          </svg>
          <div className="db-attendance-center">
            <div className="db-attendance-pct" style={{ color }}>{Math.round(presentPercent)}%</div>
          </div>
        </div>
        <div className="db-attendance-stats">
          <div className="db-attendance-stat">
            <div className="db-attendance-stat-dot" style={{ background: color }}></div>
          ...
          <div className="db-attendance-stat">
            <div className="db-attendance-stat-dot" style={{ background: '#ef4444' }}></div>
            <div className="db-attendance-stat-info">
      ...
      <div>
        ...
        <div className="db-section-header">
          ...
          <div>
            ...
            <p className="db-section-sub">Ringkasan kehadiran kegiatan terakhir</p>
          </div>
        </div>

        <div className="stats-grid" style={{ marginBottom: "2rem" }}>
          <StatCard icon="check-square" color="blue" label="Total Hadir Peserta" value={stats?.mandiriHadirPeserta ?? 0} href="/mandiri/absensi" />
          <StatCard icon="user-check" color="indigo" label="Peserta Laki-laki" value={stats?.mandiriHadirLaki ?? 0} href="/mandiri/absensi" />
          <StatCard icon="user-check" color="pink" label="Peserta Perempuan" value={stats?.mandiriHadirPerempuan ?? 0} href="/mandiri/absensi" />
          <StatCard icon="users" color="emerald" label="Total Hadir Panitia" value={stats?.mandiriHadirPanitia ?? 0} href="/mandiri/absensi" />
          <StatCard icon="user-check" color="indigo" label="Panitia Laki-laki" value={stats?.mandiriHadirPanitiaLaki ?? 0} href="/mandiri/absensi" />
          <StatCard icon="user-check" color="pink" label="Panitia Perempuan" value={stats?.mandiriHadirPanitiaPerempuan ?? 0} href="/mandiri/absensi" />
        </div>
        ...
        <div className="db-section-header">
          ...
          <div>
            ...
            <p className="db-section-sub">Rekap session results romantic room</p>
          </div>
        </div>
        ...
        <div className="stats-grid" style={{ marginBottom: "2rem" }}>
          ...
          <StatCard icon="shuffle" color="orange" label="Lanjut — Tidak" value={stats?.sessionStats?.lanjutTidak ?? 0} href="/mandiri/romantic-room" />
          <StatCard icon="shuffle" color="blue" label="Lanjut — Ragu" value={stats?.sessionStats?.lanjutRagu ?? 0} href="/mandiri/romantic-room" />
          <StatCard icon="shuffle" color="gray" label="Tidak — Ragu" value={stats?.sessionStats?.tidakRagu ?? 0} href="/mandiri/romantic-room" />
        </div>
        ...
        <div className="db-section-header">
          ...
          <div>
            ...
            <p className="db-section-sub">Visualisasi kehadiran peserta dan panitia</p>
          </div>
        </div>
        <div className="db-charts-grid">
          <AttendanceChart label="Peserta" present={stats?.mandiriHadirPeserta ?? 0} absent={stats?.mandiriTidakHadirPeserta ?? 0} color="#3b82f6" />
          <AttendanceChart label="Panitia" present={stats?.mandiriHadirPanitia ?? 0} absent={stats?.mandiriTidakHadirPanitia ?? 0} color="#10b981" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
        ...
        <>
          <div className="db-section-header">
            ...
            <div>
              ...
              <p className="db-section-sub">Statistik keanggotaan dan pendidikan</p>
            </div>
          </div>
          ...
          <div className="db-highlight-grid">
            ...
            <a href="/generus" className="db-highlight-card db-highlight-warm">
              ...
              <div className="db-highlight-sub">vs {stats?.married ?? 0} sudah menikah</div>
            </a>
          </div>
          ...
          <div className="db-edu-section">
            ...
            <div className="db-edu-grid">
              {[
                ...
                { label: "Kuliah", value: stats?.kuliah ?? 0, color: "#3b82f6" },
                { label: "Bekerja", value: stats?.bekerja ?? 0, color: "#10b981" },
              ].map(item => {
                const total = (stats?.smp ?? 0) + (stats?.sma ?? 0) + (stats?.smk ?? 0) + (stats?.kuliah ?? 0) + (stats?.bekerja ?? 0);
                const pct = total > 0 ? Math.round((item.value / total) * 100) : 0;
                return (
                  <a key={item.label} href="/generus" className="db-edu-card">
                    <div className="db-edu-bar-wrap">
                      <div className="db-edu-bar" style={{ height: `${Math.max(8, pct * 1.5)}px`, background: item.color }}></div>
                    </div>
                    <div className="db-edu-value" style={{ color: item.color }}>{item.value}</div>
      ...
      <div className="stats-grid" style={{ marginBottom: "2rem" }}>
        {(role === "admin" || role === "pengurus_daerah" || role === "kmm_daerah" || role === "desa" || role === "kelompok" || role === "tim_pnkb") && (
          <>
            <StatCard icon="calendar" color="green" label="Kegiatan Mendatang" value={stats?.kegiatan ?? 0} href="/kegiatan" gradient="linear-gradient(135deg, #22c55e 0%, #16a34a 100%)" />
        ...
        {(role === "admin" || role === "pengurus_daerah" || role === "kmm_daerah" || role === "tim_pnkb") && (
          <>
            <StatCard icon="file-text" color="orange" label="Artikel Tayang" value={stats?.artikel ?? 0} href="/admin/artikel" />
            <StatCard icon="file-text" color="red" label="Berita Tayang" value={stats?.berita ?? 0} href="/admin/berita" />
          </>
        )}
      </div>
...
function UserDashboard({ session, stats }: { session: any; stats: any }) {
  const isCreatorOrGenerus = session?.role === "creator" || session?.role === "generus";
  return (
...
function QuickActions({ role }: { role: string }) {
  const actions: { href: string; label: string; icon: React.ReactNode; variant: string; desc: string }[] = [];
  ...
  return (
    <div className="card">
      <div className="card-header">
        ...
        <span style={{ fontSize: 11, color: 'var(--text-muted)', background: 'var(--bg)', padding: '3px 10px', borderRadius: 20, fontWeight: 600 }}>{actions.length} aksi</span>
      </div>
      <div className="card-body" style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {actions.map(action => {
          ...
          return (
            <a key={action.href} href={action.href} className="db-quick-action">
              <div className="db-quick-icon" style={{ background: c.bg, color: c.color }}>
                {action.icon}
              </div>
        ...
        {actions.length === 0 && (
          <div style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: 13, padding: '16px 0' }}>Tidak ada aksi tersedia</div>
        )}
      </div>
    </div>
  );
...
function RecentInfo() {
  const infoItems = [
    { label: "Kategori Usia", value: "SMP, SMA, Kuliah, Bekerja", icon: "🎓", color: "#8b5cf6" },
    { label: "Fitur QR Code", value: "Setiap generus memiliki QR unik", icon: "📱", color: "#3b82f6" },
    { label: "Absensi", value: "Scan QR atau cari manual", icon: "✅", color: "#10b981" },
    { label: "Artikel", value: "Submit artikel, admin yang mempublish", icon: "📝", color: "#f59e0b" },
  ];
  ...
  return (
    <div className="card">
      <div className="card-header">
        ...
        <span style={{ fontSize: 11, color: 'var(--text-muted)', background: 'var(--bg)', padding: '3px 10px', borderRadius: 20, fontWeight: 600 }}>v1.0</span>
      </div>
      <div className="card-body" style={{ padding: '16px 20px' }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "0" }}>
          {infoItems.map((info, i) => (
            <div key={info.label} className="db-info-row" style={{
