...
r.post("/", requireAuth(), async (c) => {
  ...
  let attempt = 1;
  while (true) {
    const exists: any = await db.query.artikel.findFirst({ where: eq(artikel.slug, finalSlug) });
    if (!exists) break;
    finalSlug = `${rawSlug}-${++attempt}`;
  }
  await db.insert(artikel).values({ id, slug: finalSlug, judul, konten, ringkasan, coverImage, kategori: kategori || "Tuntunan Ibadah", tipe: "artikel", status: "pending", authorId: session.userId } as any);
...
r.get("/:id", optionalAuth(), async (c) => {
  ...
  const db = getDb(c.env);
  let data: any = await db.query.artikel.findFirst({ where: and(eq(artikel.slug as any, idOrSlug), eq(artikel.tipe, "artikel")) });
  if (!data) data = await db.query.artikel.findFirst({ where: and(eq(artikel.id, idOrSlug), eq(artikel.tipe, "artikel")) });
...
r.post("/:id/approve", requireAuth(), async (c) => {
  ...
  const existing: any = await db.query.artikel.findFirst({ where: and(eq(artikel.id, id), eq(artikel.tipe, "artikel")) });
  if (!existing) return c.json({ error: "Tidak ditemukan" }, 404);
  await db.update(artikel).set({ status: "published", publishedAt: new Date().toISOString(), updatedAt: new Date().toISOString() }).where(and(eq(artikel.id, id), eq(artikel.tipe, "artikel")));
  return c.json({ success: true });
});
...
r.post("/:id/reject", requireAuth(), async (c) => {
  ...
  const existing: any = await db.query.artikel.findFirst({ where: and(eq(artikel.id, id), eq(artikel.tipe, "artikel")) });
  if (!existing) return c.json({ error: "Tidak ditemukan" }, 404);
  await db.update(artikel).set({ status: "rejected", updatedAt: new Date().toISOString() }).where(and(eq(artikel.id, id), eq(artikel.tipe, "artikel")));
  return c.json({ success: true });
});
