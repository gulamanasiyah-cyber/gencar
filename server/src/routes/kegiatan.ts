...
r.get("/", async (c) => {
  ...
  if (session.role === "desa" && session.desaId) conditions.push(eq(kegiatan.desaId, session.desaId));
...
r.post("/", async (c) => {
  ...
  if (session.role === "desa" && !desaId) finalDesaId = session.desaId;
...
r.put("/:id", async (c) => {
  ...
  if (session.role === "desa" && existing.desaId !== session.desaId) return c.json({ error: "Forbidden" }, 403);
  if (session.role === "kelompok" && existing.kelompokId !== session.kelompokId) return c.json({ error: "Forbidden" }, 403);
