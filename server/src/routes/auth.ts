...
auth.post("/register", async (c) => {
  ...
  const { name, email, password, desaId, kelompokId, jenisKelamin, kategoriUsia, kategori: _kategori, namaOrtu, tempatLahir, tanggalLahir, noTelp, noTelpOrtu, alamat, foto } = result.data as any;
  const db = getDb(c.env);
  ...
  await db.insert(generus).values({ id: generusId, nomorUnik, nama: name, jenisKelamin: jenisKelamin === "P" ? "P" : "L", kategoriUsia: (kategoriUsia || "Mandiri") as any, kategori: "Generus", namaOrtu, tempatLahir, tanggalLahir, alamat, noTelp, noTelpOrtu, desaId: Number(desaId), kelompokId: Number(kelompokId), isGenerus: 1, foto: foto || null } as any);
  await db.insert(users).values({ id, name, email: email.toLowerCase(), passwordHash, passwordPlain, role: assignedRole, desaId: Number(desaId), kelompokId: Number(kelompokId), generusId } as any);
...
auth.get("/kelompok", async (c) => {
  try {
    ...
    console.error("[GET /api/auth/kelompok]", e);
    return c.json([], 200);
  }
});
