...
r.get("/", async (c) => {
  ...
  const roleDesaId = session.role === "admin_desa" && session.desaId ? session.desaId : null;
  const roleKelompokId = session.role === "admin_kelompok" && session.kelompokId ? session.kelompokId : null;
  const effectiveDesaId = roleDesaId ?? qDesaId;
  const effectiveKelompokId = roleKelompokId ?? qKelompokId;

  const generusConds: any[] = [];
  generusConds.push(eq(generus.isGenerus, 1));
  generusConds.push(sql`${generus.id} NOT IN (SELECT generus_id FROM users WHERE generus_id IS NOT NULL AND role IN ('admin_daerah', 'admin_desa', 'admin_kelompok'))`);
