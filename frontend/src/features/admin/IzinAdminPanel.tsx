  ...
  return (
    <div>
      <div className="kpi">
        ...
        <KpiCard icon={<span className="kpi-icon kpi-icon--emerald"><IcoCal size={18} /></span>} label="Kegiatan mendatang" value={rows.filter((r) => r.tanggal >= new Date().toISOString().slice(0, 10)).length} />
      </div>
          ...
          {rows.map((r) => (
            <div key={r.id} className="card" style={{ padding: 14, display: "grid", gap: 8 }}>
              <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                ...
                <span className="pill pill-amber" style={{ fontSize: 10 }}>Menunggu</span>
              </div>
